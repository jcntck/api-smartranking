...
export class DesafiosService {
    ...
    async consultarDesafiosDeUmJogador(idJogador: string): Promise<Array<Desafio>> {
    ...
    async criarDesafio(criarDesafioDto: CriarDesafioDto): Promise<any> {
        ...
        if (criarDesafioDto.jogadores.filter(jogador => jogador._id === solicitante).length === 0) {
            throw new BadRequestException('O solicitante precisa ser um dos jogadores do desafio');
        }

        const categoria = await this.categoriasService.consultarCategoriaPeloJogador(solicitante);
        if (!categoria) {
            throw new NotFoundException(`O jogador ${solicitante} não está cadastrado em nenhuma categoria`);
        }
    ...
    async atribuirPartidaDesafio(_id: string, atribuirPartidaDesafioDto: AtribuirPartidaDesafioDto): Promise<void> {
        const desafioEncontrado = await this.desafioModel.findById(_id);
        if (!desafioEncontrado) {
            throw new BadRequestException(`Desafio ${_id} não cadastrado`);
        }

        const jogadorFilter = desafioEncontrado.jogadores.filter(jogador => jogador._id == atribuirPartidaDesafioDto.def);
        if (jogadorFilter.length == 0) {
